(ns gb_standards.main
  "Kotodama WASM entrypoint for the GB Standards clean-room actor (L4) — Clojure port.
  ...
  facts (`gb_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gb_standards")
(def tier "L4")
...
(def entity-specs
  [{:entity "Substance" :plural "substances" :id-prefix "gbstanda_sub"
    :fields [:casNumber :name :formula :molarMass] :required [:casNumber :name]
    :coerce {:molarMass :float}           :refs {}}
   {:entity "Compound"  :plural "compounds"  :id-prefix "gbstanda_com"
    :fields [:inchiKey :smiles :name]                   :required [:inchiKey :smiles]
    :coerce {}                            :refs {}}
   {:entity "Structure" :plural "structures" :id-prefix "gbstanda_str"
    :fields [:pdbId :title :method :resolution]        :required [:title :method]
    :coerce {:resolution :float}          :refs {}}
   {:entity "Material"  :plural "materials"  :id-prefix "gbstanda_mat"
    :fields [:materialId :formula :bandGap]            :required [:formula :bandGap]
    :coerce {:bandGap :float}             :refs {:materialId "Material"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "gbstanda_sta"
    :fields [:designation :title :organization]        :required [:designation :title]
    :coerce {}                            :refs {}}
   {:entity "Property"  :plural "properties" :id-prefix "gbstanda_pro"
    :fields [:substanceId :name :value :unit]          :required [:name :value]
    :coerce {:value :float}               :refs {:substanceId "Substance"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gb_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gb_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gb_standards-compat/ready)
